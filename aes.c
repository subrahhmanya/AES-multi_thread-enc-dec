...
//#define _FILE_OFFSET_BITS 64
 ...
 #define THREADS_BUFF_SIZE 1024*1024*8
 #define  FILE_SIZE  1024*1024
 pthread_mutex_t lseek_mut,read_mut,write_mut;
 struct thread_block
 {
    FILE *fin;
    FILE *fout;
 ...
 void *thread_encrypt_fn(void *arg)
{  
    struct thread_block *block = (struct thread_block *)arg;  
    unsigned char *buf;
    buf = (unsigned char *)malloc(THREADS_BUFF_SIZE * sizeof(char));      
    if(NULL == buf)
    {
        printf("malloc fail,errno is =%d,error is =%s\n",errno,strerror(errno));
        exit(1);
    }
    int ret;  
    size_t count = block->start;  
    aes_context ctx;
    aes_set_key(&ctx,block->keys,128);
    unsigned long read_position = 0;
    unsigned long write_position = 0;
    read_position = block->start;
    write_position = block->start;
    unsigned long bytes_read;  
    unsigned long bytes_write;  
    int i =0;
    unsigned long testnum = 0;
    unsigned long read_long = 0;
    int number = 0;
    while(count < block->end)  
    {  
        if (count + THREADS_BUFF_SIZE < block->end)
        {
            read_long = THREADS_BUFF_SIZE;
        }
        else
        {
            read_long = block->end - count;
        }
        pthread_mutex_lock(&read_mut);
        ret = fseek(block->fin,read_position,SEEK_SET);  
        bytes_read = fread(buf,sizeof(char),read_long,block->fin);
        read_position = read_position + bytes_read;
        ...
        ret = fseek(block->fout,write_position,SEEK_SET);
        bytes_write=fwrite(buf,sizeof(char),bytes_read,block->fout);
        write_position = write_position + bytes_write;
        pthread_mutex_unlock(&write_mut);
        
        if(bytes_write != bytes_read){ printf("write error\n");}
        }  
...
*/
extern "C"
int encrypt_file(unsigned char *key,const char *des_file_path,const char * src_file_path,unsigned long length)
{
   // const int g_ncore = get_CPU_core_num();
   // printf("g_ncore is %d\n",g_ncore);
    struct stat buf;
    stat(src_file_path,&buf);
    ...
    if (file_size < FILE_SIZE)
    {
        ...
        FILE *fout = fopen(des_file_path,"wb");
        if(fin == NULL || fout ==NULL)
        {
            printf("file open error,errno value:%d,it means:%s\n",errno,strerror(errno));
            return -1;
        }
        int len=0;
        unsigned long i =0;
        while( (len=fread(buffer,sizeof(char),length,fin)) != 0)
        {
            
            for(i=0;i!=len;i+=16)
            {
                aes_encrypt(&ctx, buffer+i,buffer+i);
            }
    ...
    else
    {
       ...
       // printf("THREADS_COUNT is =%d\n",THREADS_COUNT);
        ...
        for(i = 0 ; i < thread_size; ++i)  
            {
                int err;
                err =pthread_create(&ptid[i],NULL,thread_encrypt_fn,&(blocks[i])); 
...
void *thread_decrypt_fn(void *arg)
{  
    struct thread_block *block = (struct thread_block *)arg;  
    unsigned char *buf;
    buf = (unsigned char *)malloc(THREADS_BUFF_SIZE * sizeof(char));      
...
extern "C"
int decrypt_file(unsigned char *key,const char *des_file_path,const char * src_file_path,unsigned long length)
{
    struct stat buf;
    stat(src_file_path,&buf);
    size_t file_size = buf.st_size;
    ...
    else
    {
       ...
       //    int  THREADS_COUNT = 4;
        ...
        for(; i < thread_size;++i)  
         ...
         blocks[i-1].end = file_size;//the last thread deal 
        pthread_t ptid[thread_size];
        // lock init
        pthread_mutex_init(&read_mut,NULL);
        pthread_mutex_init(&write_mut,NULL);
       ...
       for(i = 0 ; i < thread_size; ++i)  
            {
                int err;
                err =pthread_create(&ptid[i],NULL,thread_decrypt_fn,&(blocks[i])); 
                if(err != 0){
                    printf("can't create thread:%s\n",strerror(err));
                }
            }  
